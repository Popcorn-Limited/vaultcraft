...
async function simulateCall({ address, account, abi, args, functionName, publicClient }: any): Promise<SimulationResponse> {
  try {
...
function SetOptionTokenOracleParams(): JSX.Element {
  ...
  const [values, setValues] = useState<{ multiplier: number, secs: number, ago: number, minPrice: number }>({ multiplier: 0, secs: 0, ago: 0, minPrice: 0 })
  ...
  async function getValues() {
    ...
    setValues({ multiplier: Number(res[0]), secs: Number(res[1]), ago: Number(res[2]), minPrice: Number(res[3]) })
  }
  ...
  function handleChangeInput(value: string, key: "multiplier" | "secs" | "ago" | "minPrice") {
        ...
        <div>
          ...
          <div className="border border-gray-500">
            <InputNumber
              ...
              onChange={e => handleChangeInput(e.currentTarget.value, "multiplier")}
            />
          </div>
        </div>
        <div>
        ...
        <div>
          ...
          <div className="border border-gray-500">
            <InputNumber
              ...
              onChange={e => handleChangeInput(e.currentTarget.value, "minPrice")}
            />
          </div>
        </div>
      </div>
      <div className="w-40 mt-4">
        <MainActionButton label="Submit" handleClick={() => handleMainAction()} disabled={!account} />
      </div>
    </div>
