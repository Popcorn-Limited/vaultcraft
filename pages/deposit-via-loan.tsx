...
//     fetchReserveData( publicClient).then(
...
export function AaveUserAccountData({ supplyToken, borrowToken }: { supplyToken: Token, borrowToken: Token }): JSX.Element {
  ...
  useEffect(() => {
    if (publicClient) fetchReserveData(publicClient).then(
      res => setReserveData(res)
    )
  }, [publicClient])
    ...
    <>
      <div className="w-full flex justify-between gap-8 md:gap-4">
        <div className="w-full mt-6 md:mt-0">
          <p className="text-primary font-normal md:text-[14px]">Lending Apy</p>
          <span className="flex flex-row items-center space-x-2">
            <TokenIcon token={supplyToken} icon={supplyToken.logoURI} chainId={10} imageSize={"w-6 h-6 mb-0.5"} />
            <Title level={2} fontWeight="font-normal" as="span" className="mr-1 text-primary">
              {formatToFixedDecimals(reserveData.find(d => d.asset === supplyToken.address)?.supplyRate || 0, 2)} %
            </Title>
          </span>
        </div>

        <div className="w-full mt-6 md:mt-0">
          <p className="text-primary font-normal md:text-[14px]">Borrow Apy</p>
          <span className="flex flex-row items-center space-x-2">
            <TokenIcon token={borrowToken} icon={borrowToken.logoURI} chainId={10} imageSize={"w-6 h-6 mb-0.5"} />
            <Title level={2} fontWeight="font-normal" as="span" className="mr-1 text-primary">
              {formatToFixedDecimals(reserveData.find(d => d.asset === borrowToken.address)?.borrowRate || 0, 2)} %
            </Title>
          </span>
        </div>
      ...
      <div className="w-full flex justify-between gap-8 md:gap-4 mt-4">
        <div className="w-full mt-6 md:mt-0">
          ...
          <Title as="span" level={2} fontWeight="font-normal" className="text-primary">
            {reserveData.find(d => d.asset === supplyToken.address)?.ltv.toFixed(2)} %
          </Title>
        </div>
        ...
        <div className="w-full mt-6 md:mt-0">
          ...
          <Title as="span" level={2} fontWeight="font-normal" className="text-primary">
            {reserveData.find(d => d.asset === supplyToken.address)?.liquidationThreshold.toFixed(2)} %
          </Title>
        </div>
        ...
        <div className="w-full mt-6 md:mt-0">
          ...
          <Title as="span" level={2} fontWeight="font-normal" className="text-primary">
            {reserveData.find(d => d.asset === supplyToken.address)?.liquidationPenalty.toFixed(2)} %
          </Title>
        </div>
      </div>
