...
export default async function getGauges(
  ...
  const finalGaugeData: GaugeData = {};

  // @ts-ignore
  for (let [i, gauge] of gauges.entries()) {
    finalGaugeData[gauge] = await getGaugeApr(Number(gaugeTypes[i]), gauge, vcxPriceInUsd)
  }
  return finalGaugeData;
}
...
async function getGaugeApr(gaugeType: number, gauge: Address, vcxPriceInUsd: number) {
