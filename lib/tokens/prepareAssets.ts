...
import {
  vcx as getVcxPrice,
  vcxLp as getVcxLpPrice,
} from "@/lib/resolver/price/resolver";
import {
  ALT_NATIVE_ADDRESS,
  OptionTokenByChain,
  ST_VCX,
...
export async function prepareAssets(
  ...
  const { data: priceData } = await axios.get(
    `https://pro-api.llama.fi/${
      process.env.DEFILLAMA_API_KEY
    }/coins/prices/current/${String(
      addresses.map(
  ...
  addresses.forEach((address, i) => {
    let tokenPrice =
      Number(
        priceData.coins[`${networkMap[chainId].toLowerCase()}:${address}`]
          ?.price
      ) || 0;

    tokenPrice = handleTokenPriceException(
      address,
      tokenPrice,
      vcxPrice,
      vcxLpPrice,
    ...
    result[getAddress(address)] = {
      ...
      totalSupply:
        address === ALT_NATIVE_ADDRESS ? maxUint256 : (ts[i].result as bigint),
      chainId: chainId,
...
function handleTokenPriceException(
  address: Address,
  tokenPrice: number,
  vcxPrice: number,
  vcxLpPrice: number,
...
async function handleChainException(
  addresses: Address[],
  assets: any,
  chainId: number,
  client: PublicClient
  ...
  const { data: btcPriceData } = await axios.get(
    `https://pro-api.llama.fi/${process.env.DEFILLAMA_API_KEY}/coins/prices/current/ethereum:0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599`
  );

  let result: TokenByAddress = {};
  addresses.forEach((address, i) => {
    let tokenPrice = Number(
      btcPriceData.coins["ethereum:0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599"]
        ?.price
    );
