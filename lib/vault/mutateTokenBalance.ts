...
export default async function mutateTokenBalance({
  ...
  if (NETWORKS_SUPPORTING_ZAP.includes(chainId)) {
    const zapAssetFound = zapAssets[chainId].find(
      (asset) => asset.address === inputToken || asset.address === outputToken
    ); // @dev -- might need to copy the state here already to avoid modifing a pointer
    if (zapAssetFound) {
      zapAssetFound.balance =
        zapAssetFound.address === inputToken
          ? Number(data[0])
          : Number(data[1]);
      setZapAssets({
        ...zapAssets,
        [chainId]: [...zapAssets[chainId], zapAssetFound],
      });
    }
  }
  ...
  newVaultState.forEach((vaultData) => {
    if (vaultData.chainId === chainId) {
      ...
      if (vaultData.address === vault) {
        ...
        vaultData.tvl =
          (Number(data[3]) * pricePerShare) / 10 ** vaultData.asset.decimals;
      ...
      if (
        vaultData.vault.address === inputToken ||
        vaultData.vault.address === outputToken
      ) {
      ...
      if (
        vaultData.asset.address === inputToken ||
        vaultData.asset.address === outputToken
      ) {
      ...
      if (
        vaultData.gauge?.address === inputToken ||
        vaultData.gauge?.address === outputToken
      ) {
        vaultData.gauge.balance =
          vaultData.gauge.address === inputToken
            ? Number(data[0])
            : Number(data[1]);
      }
