...
async function getSafeVaultApy(vault: VaultData): Promise<LlamaApy[]> {
  ...
  let fromBlock: bigint | BlockTag | undefined;
  let deployBlock = ORACLES_DEPLOY_BLOCK[vault.chainId];
  if (deployBlock > 0)
    if (deployBlock <= Number(blockNumber) - 100000) // max 100000 block
      fromBlock = blockNumber - BigInt(99000);
    else
  ...
  logs.slice(1).forEach(async (log: any, i: number) => {
    ...
    const averagedApy = (entries.reduce((acc, entry) => acc + (entry.apy / 100), 0) + annualizedReturn) / (entries.length + 1)

    // push data point
    entries.push({
      apy: averagedApy * 100,
      apyBase: averagedApy * 100,
      apyReward: 0,
      tvl: vault.tvl,
