...
export const curveApiCall = async ({
  ...
  baseAsset: string;
  router: string;
  toBaseAssetRoutes: CurveRoute[];
  toAssetRoute: CurveRoute;
  minTradeAmounts: BigInt[];
  optionalData: string;
}> => {
  ...
  for (let i = 0; i < rewardTokens.length; i++) {
    ...
    const { route: rewardRoute } = await curve.router.getBestRouteAndOutput(
      inputToken,
      outputToken,
      "100000000"
    );
  ...
  if (baseAsset.toLowerCase() !== depositAsset.toLowerCase()) {
    const { route: assetRoute } = await curve.router.getBestRouteAndOutput(
      baseAsset,
      depositAsset,
      "100000000"
    );
...
export const curveApiCallToBytes = async ({
  ...
  const curveData = await curveApiCall({
    depositAsset,
    rewardTokens,
    baseAsset,
    router,
