...
export async function resolveStrategyDefaults({
  chainId,
  client,
  address,
  resolver,
}: StrategyDefaultResolverParams & { resolver?: string }): Promise<any[]> {
  if (chainId === localhost.id) chainId = mainnet.id;

  try {
    return resolver
      ? StrategyDefaultResolvers[resolver]({
          chainId,
          client,
          address: getAddress(address),
        })
      : StrategyDefaultResolvers.default({
          chainId,
          client,
          address: getAddress(address),
        });
