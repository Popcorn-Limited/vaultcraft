...
async function createRoute(
  ...
  const route: BalancerRoute = {
    ...
    limits: res.tokenAddresses.map((address, i) =>
      i < res.tokenAddresses.length - 1 ? MAX_INT256 : BigInt("-1")
    ),
  };
...
export async function balancerLpCompounder({
  chainId,
  client,
  address,
  params,
  ...
  ); // TODO - fetch gas price dynamically
  const values = [route1];
...
const balancerOptionalDataAbi = [
  { name: "poolId", type: "bytes32" },
  { name: "indexIn", type: "uint8" },
  { name: "amountsInLen", type: "uint8" },
] as const;
