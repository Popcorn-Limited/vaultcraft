...
export default function SelectField({
  ...
  onChange: (option: { image?: string; label: string }) => void;
}): JSX.Element {
  const [isOpened, setIsOpened] = useState(false);
  const changeValue = (option: { image?: string; label: string }) => {
    onChange(option);
    setIsOpened(false);
  };
  ...
  return (
    <div className={`relative flex items-center ${className} myClass`}>
      ...
      {isOpened && (
        <div
          className={`absolute flex flex-col w-[16rem] p-2 right-0 top-[100%] z-100 bg-[#23262F] rounded-[0.75rem] border-[0.125rem] border-[#353945]`}
        >
          {options.map((option, index) => {
            return (
              ...
              >
                ...
                <div
                  className={
                    option.label === value.label ? "text-black" : "text-white"
                  }
                >
                  {option.label}
                </div>
