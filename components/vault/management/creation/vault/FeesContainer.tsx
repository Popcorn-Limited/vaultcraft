...
export function isFeeConfigValid(fees: any): boolean {
  if (
    ...
    return false;
  if (
    !isAddress(fees.recipient) ||
    fees.recipient === zeroAddress ||
    fees.recipient.length === 0
  )
    return false;
  return true;
}
    ...
    <VaultCreationCard activeStage={activeStage} stages={stages}>
      <div className={`mb-6`}>
        ...
        <p className="text-white">
          ...
          out in shares of the vault. Fees can be changed at any time after
          vault creation. (Use . for fees with decimals)
        </p>
      </div>
      ...
      <div className="flex justify-end mt-8 gap-3">
        ...
        <MainActionButton
          label="Next"
          handleClick={() => router.push(`/create-vault/${route}/review`)}
          disabled={!isFeeConfigValid(fees)}
        />
      </div>
    </VaultCreationCard>
  );
