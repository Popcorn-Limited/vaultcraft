...
export default function VaultInteraction({
...
}: VaultInteractionProps): JSX.Element {
  ...
  const [days, setDays] = useState<string>(
    String(vaultData.lock.daysToUnlock > 0 ? vaultData.lock.daysToUnlock : "7")
  );

  const isDeposit =
    vaultData.lock.unlockTime === 0 || vaultData.lock.daysToUnlock > 0;

  const [availableTabs, setAvailableTabs] = useState<string[]>(
    depositDisabled ? ["Deposit"] : ["Deposit", "Claim"]
  );
  const [activeTab, setActiveTab] = useState<string>("Deposit");

  const [stepCounter, setStepCounter] = useState<number>(0);
  const [steps, setSteps] = useState<ActionStep[]>([]);
  const [action, setAction] = useState<LockVaultActionType>(
    LockVaultActionType.Deposit
  );
  ...
  useEffect(() => {
    ...
    if (isDeposit) {
      const newAction =
        vaultData.lock.unlockTime === 0
          ? LockVaultActionType.Deposit
          : LockVaultActionType.IncreaseAmount;
      setAction(newAction);
      ...
      setInputBalance(
        String(vaultData.lock.amount / 10 ** vaultData.vault.decimals)
      );
  ...
  function handleTokenSelect(input: Token, output: Token): void {
    ...
    switch (input.address) {
      ...
      default:
        const newAction =
          vaultData.lock.unlockTime === 0
            ? LockVaultActionType.ZapDeposit
            : LockVaultActionType.ZapIncreaseAmount;
        setAction(newAction);
        setSteps(getLockVaultActionSteps(newAction));
        break;
    }
