...
export default function Deposit({ vaultData, inputBalState, daysState }: DepositProps): JSX.Element {
  ...
  function handleMaxClick() {
    ...
    const stringBal = vaultData.asset.balance.toLocaleString("fullwide", { useGrouping: false })
  ...
  return <>
    ...
    <InputTokenWithError
      ...
      onMaxClick={() => { }}
      chainId={vaultData.chainId}
      value={(Number(inputBalance) * (Number(vaultData.asset?.price)) / Number(vaultData.vault?.price)) || 0}
      onChange={() => { }}
