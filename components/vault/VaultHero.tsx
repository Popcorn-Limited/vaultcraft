          ...
          {vaultData.gaugeData?.lowerAPR ? (
            <div>
              <LargeCardStat
                ...
                label="Min Boost APR"
                value={`${NumberFormatter.format(
                  roundToTwoDecimalPlaces(vaultData.gaugeData?.lowerAPR)
                )} %`}
                tooltip={`Minimum oVCX boost APR based on most current epoch's distribution. (Based on the current emissions for this gauge of ${NumberFormatter.format(
                  vaultData?.gaugeData.annualEmissions / 5
                )} oVCX p. year)`}
              />
            </div>
          ) : (
          ...
          {vaultData.gaugeData?.upperAPR ? (
            <div>
              <LargeCardStat
                ...
                label="Max Boost APR"
                value={`${NumberFormatter.format(
                  roundToTwoDecimalPlaces(vaultData.gaugeData?.upperAPR)
                )} %`}
                tooltip={`Maximum oVCX boost APR based on most current epoch's distribution. (Based on the current emissions for this gauge of ${NumberFormatter.format(
                  vaultData?.gaugeData.annualEmissions
                )} oVCX p. year)`}
              />
            </div>
          ) : (
          ...
          {vaultData.gaugeData?.rewardApy.apy ? (
            <div>
              <LargeCardStat
                ...
                label="Add. Rewards"
                value={`${NumberFormatter.format(
                  roundToTwoDecimalPlaces(vaultData.gaugeData?.rewardApy.apy)
                )} %`}
                tooltipChild={
                  <div className="w-28">
