    ...
    <>
      <div className="w-full flex justify-between gap-8 md:gap-4">
        <div className="w-full mt-6 md:mt-0">
          <p className="text-primary font-normal md:text-[14px]" id={`${baseTooltipId}-wallet`}>Your Wallet</p>
          ...
          <ResponsiveTooltip
            ...
            content={<p className="max-w-52">Deposit assets held in your wallet</p>}
          />
        </div>
        ...
        <div className="w-full mt-6 md:mt-0">
          <p className="leading-6 text-primary md:text-[14px]" id={`${baseTooltipId}-tvl`}>TVL</p>
        ...
        <ResponsiveTooltip
          ...
          content={<p className="max-w-52">Total value of all assets deposited into the vault</p>}
        />
      </div>

      <div className="w-full flex flex-row justify-between gap-8 md:gap-4">
        <div className="w-full mt-6 md:mt-0">
          <p className="font-normal text-primary md:text-[14px]" id={`${baseTooltipId}-vApy`}>vAPY</p>
          <Title as="span" level={2} fontWeight="font-normal" className="text-primary">
            {apy ? `${NumberFormatter.format(roundToTwoDecimalPlaces(apy))} %` : "0 %"}
          </Title>
          <ResponsiveTooltip
            id={`${baseTooltipId}-vApy`}
            content={<p className="max-w-52">Current variable apy of the vault</p>}
          />
        </div>
        {isLockVault ?
          <LockRewards vaultData={vaultData as LockVaultData} baseTooltipId={baseTooltipId} />
...
function LockRewards({ vaultData, baseTooltipId }: { vaultData: LockVaultData, baseTooltipId: string }): JSX.Element {
  const totalRewardApy = vaultData.rewards.reduce((a, b) => a + b.rewardApy, 0)

  return <>
    <div className="w-full mt-6 md:mt-0">
      <p className="font-normal text-primary md:text-[14px]" id={`${baseTooltipId}-minReward`}>Min Rewards</p>
      <Title as="span" level={2} fontWeight="font-normal" className="text-primary">
        {totalRewardApy ? (totalRewardApy / 4).toFixed(2) : "-"} %
      </Title>
      <ResponsiveTooltip
        id={`${baseTooltipId}-minReward`}
        content={<p className="max-w-52">Minimum reward APR based on most recent distribution event</p>}
      />
    </div>

    <div className="w-full mt-6 md:mt-0">
      <p className="font-normal text-primary md:text-[14px]" id={`${baseTooltipId}-maxReward`}>Max Rewards</p>
      <Title as="span" level={2} fontWeight="font-normal" className="text-primary">
        {totalRewardApy ? totalRewardApy.toFixed(2) : "-"} %
      </Title>
      <ResponsiveTooltip
        id={`${baseTooltipId}-maxReward`}
        content={<p className="max-w-52">Maximum reward APR based on most recent distribution event</p>}
      />
    </div>
  </>
}

function GaugeRewards({ vaultData, baseTooltipId }: { vaultData: VaultData | LockVaultData, baseTooltipId: string }): JSX.Element {
  return <>
    <div className="w-full mt-6 md:mt-0">
      <p className="font-normal text-primary md:text-[14px]" id={`${baseTooltipId}-minBoost`}>Min Boost</p>
      <Title as="span" level={2} fontWeight="font-normal" className="text-primary">
    ...
    <div className="w-full mt-6 md:mt-0">
      <p className="font-normal text-primary md:text-[14px]" id={`${baseTooltipId}-maxBoost`}>Max Boost</p>
      <Title as="span" level={2} fontWeight="font-normal" className="text-primary">
